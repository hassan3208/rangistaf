          ...
          {[...orders]
            ...
            >
              ...
              <div className="text-sm font-medium mt-2">
                {(() => {
                  const subtotal = o.products.reduce((sum, i) => {
                    const qty = i.quantity || 1;
                    const unit = Number(i.price) / qty;
                    const discounted = i.discount
                      ? unit - (unit * i.discount) / 100
                      : unit;
                  ...
                  else if (o.status === "processing" || o.status === "shipped") {
                    paymentInfo =
                      `Paid: ${formatPKR(Math.round(finalTotal * 0.25))} | ` +
                      `Remaining: ${formatPKR(Math.round(finalTotal * 0.75))}`;
                  }
                  else if (o.status === "delivered") {
                    paymentInfo = `Paid: ${formatPKR(finalTotal)}`;
                  }

                  const totalQty = o.products.reduce((s, i) => s + i.quantity, 0);
                  const dc = subtotal >= 5000 || totalQty >= 3 ? 0 : 200;

                  return (
                    <>
                      <p className="text-muted-foreground">{paymentInfo}</p>
                      <p className="text-muted-foreground">Delivery Charge: {dc === 0 ? "Free" : `Rs ${dc}`}</p>
                    </>
                  );
                })()}
              </div>
